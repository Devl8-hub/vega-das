...
class RENDER_PT_format(RenderOutputButtonsPanel, Panel):
    ...
    @classmethod
    def poll(cls, context):
        return (context.engine in cls.COMPAT_ENGINES) and (context.engine != 'CYCLES')
...
class RENDER_PT_encoding(RenderOutputButtonsPanel, Panel):
    ...
    def poll(cls, context):
        rd = context.scene.render
        return (rd.image_settings.file_format in {'FFMPEG', 'XVID', 'H264', 'THEORA'}) and (context.engine != 'CYCLES')
