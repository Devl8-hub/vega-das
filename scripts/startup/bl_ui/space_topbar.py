...
class TOPBAR_HT_upper_bar(Header):
    ...
    def draw_left(self, context):
        ...
        if not screen.show_fullscreen:
            ...
            # MINIMAL UI: Workspace switcher removed
            # row = layout.row(align=True)
            # for name in ["Layout", "UV Editing", "Shading"]:
            #     # Only draw if the workspace actually exists
            #     ws = bpy.data.workspaces.get(name)
            #     if ws:
            #         row.operator("workspace.set_minimal", text=name, depress=(window.workspace == ws)).name = name
            pass
    ...
    def draw_right(self, context):
        ...
        # MINIMAL UI: Scene and ViewLayer controls removed
...
class TOPBAR_MT_editor_menus(Menu):
    ...
    def draw(self, context):
        ...
        #    layout.menu("TOPBAR_MT_blender", text="Vega DAS", icon='NONE')
        # else:
        #    layout.menu("TOPBAR_MT_blender", text="Vega DAS", icon='NONE')
...
class TOPBAR_MT_blender(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("wm.splash_vega", text="", icon='NONE')
        layout.operator("wm.about_vega", text="About Vega DAS", icon='NONE')

        # layout.separator()
        # layout.operator("preferences.app_template_install", text="Install Application Template...")
        # layout.separator()
        # layout.menu("TOPBAR_MT_blender_system", text="System", icon='NONE')
...
class TOPBAR_MT_file_defaults(Menu):
    ...
    def draw(self, context):
        ...
        if app_template:
            ...
            props = layout.operator("wm.read_factory_settings", text="Load Factory Default Settings")
...
# -------------------------------------
# Minimal Workspace Switcher Operator
# -------------------------------------

class WORKSPACE_OT_set_minimal(bpy.types.Operator):
    """Set the active workspace"""
    bl_idname = "workspace.set_minimal"
    bl_label = "Set Workspace"
    bl_options = {'INTERNAL'} # Hide from search

    name: bpy.props.StringProperty(name="Workspace Name")
