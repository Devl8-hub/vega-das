 ...
 * \ -dump | grep vega-das-thumbnailer
 *
 * # Debugging
 ...
 * are: qlmanage, quicklookd, kernel, vega-das-thumbnailer, secinitd,
 ...
 *   xattr -rl bin/Vega DAS.app/Contents/Plugins/vega-das-thumbnailer.appex
 * - Is it registered with lsregister and there isn't a conflict with another plugin taking
 *   precedence? lsregister -dump | grep vega-das-thumbnailer.appex
 * - For RBSLaunchRequest error: is the executable flag set? chmod u+x
 * bin/Vega DAS.app/Contents/PlugIns/vega-das-thumbnailer.appex/Contents/MacOS/vega-das-thumbnailer
 * - Is it codesigned and sandboxed?
 *   codesign --display --verbose --entitlements - --xml \
 * bin/Vega DAS.app/Contents/Plugins/vega-das-thumbnailer.appex codesign --deep --force --sign - \
 * --entitlements ../blender/release/darwin/thumbnailer_entitlements.plist --timestamp=none \
 * bin/Vega DAS.app/Contents/Plugins/vega-das-thumbnailer.appex
 * - Sometimes vega-das-thumbnailer running in background can be killed.
 ...
 public:
  ...
  ~FileDescriptorRAII()
  {
    if (good()) {
      int ok = close(src_fd);
      if (!ok) {
        NSLog(@"Vega DAS Thumbnailer Error: Failed to close the blend file.");
      }
    }
  }
...
{
  NSLog(@"Vega DAS Thumbnailer Error: %@", errorStr);
  return [NSError errorWithDomain:@"com.vegadas.blender.thumbnailer"
                             code:-1
