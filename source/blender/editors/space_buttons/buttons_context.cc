...
void buttons_context_compute(const bContext *C, SpaceProperties *sbuts)
{
  ...
  /* Force "Vega DAS" contexts to be valid, even if no object is selected.
   * This prevents the UI from resetting to Scene/Render when switching to these tabs. */
  flag |= (1 << BCONTEXT_OBJECT);
  flag |= (1 << BCONTEXT_DATA);
  flag |= (1 << BCONTEXT_MATERIAL);
  flag |= (1 << BCONTEXT_RENDER);
