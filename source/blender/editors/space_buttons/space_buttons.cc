...
static SpaceLink *buttons_create(const ScrArea * /*area*/, const Scene * /*scene*/)
{
  ...
  sbuts->mainb = sbuts->mainbuser = BCONTEXT_DATA;
  sbuts->visible_tabs = (1 << BCONTEXT_OBJECT) | (1 << BCONTEXT_DATA) | (1 << BCONTEXT_MATERIAL) |
                        (1 << BCONTEXT_RENDER);
  ...
  region->flag |= RGN_FLAG_HIDDEN; /* Force hidden. */
  region->sizex = 0;               /* Zero width. */
...
static bool buttons_navigation_bar_region_poll(const RegionPollParams * /*params*/)
{
  return false;
}
...
Vector<eSpaceButtons_Context> ED_buttons_tabs_list(const SpaceProperties *sbuts, bool apply_filter)
{
  ...
  auto add_tab = [&](eSpaceButtons_Context tab) {
    /* Force Render to be visible regardless of user preferences/filter */
    bool is_visible = (!apply_filter || (filter & (1 << tab)));
    bool is_valid = (sbuts->pathflag & (1 << tab));

    if (tab == BCONTEXT_RENDER) {
      is_visible = true;
      is_valid = true; /* Force valid even if buttons_context_compute hasn't run yet */
    }
  ...
  /* Force OBJECT, DATA, MATERIAL for the UI dropdown. */
  if (!tabs.contains(BCONTEXT_OBJECT)) {
    tabs.append(BCONTEXT_OBJECT);
  }
  if (!tabs.contains(BCONTEXT_DATA)) {
    tabs.append(BCONTEXT_DATA);
  }
  if (!tabs.contains(BCONTEXT_MATERIAL)) {
    tabs.append(BCONTEXT_MATERIAL);
  }
...
static void buttons_main_region_layout(const bContext *C, ARegion *region)
{
  ...
  sbuts->visible_tabs |= (1 << BCONTEXT_RENDER);
...
void ED_spacetype_buttons()
{
  ...
  art->poll = buttons_navigation_bar_region_poll;
