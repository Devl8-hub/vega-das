...
static void view3d_init(wmWindowManager * /*wm*/, ScrArea *area)
{
  ...
  /* Enforce opaque settings for Vega DAS to prevent confusion in Edit Mode */
  v3d->shading.flag &= ~V3D_SHADING_XRAY;
  v3d->shading.xray_alpha = 1.0f;
}
...
static bool view3d_tools_region_poll(const RegionPollParams *params)
{
  const bContext *C = params->context;
...
void ED_spacetype_view3d()
{
  ...
  art->poll = view3d_tools_region_poll;
  BLI_addhead(&st->regiontypes, art);
