...
static void rna_RenderSettings_engine_set(PointerRNA *ptr, int value)
{
  ...
  if (type) {
    ...
    RenderEngineType *t;
    for (t = static_cast<RenderEngineType *>(R_engines.first); t; t = t->next) {
      if (t->idname &&
          (BLI_strcaseeq(t->idname, "CYCLES") || BLI_strcaseeq(t->idname, "CYCLES_RENDER")))
      {
        has_cycles = true;
        break;
      }
    ...
    bool is_cycles = type->idname && (BLI_strcaseeq(type->idname, "CYCLES") ||
                                      BLI_strcaseeq(type->idname, "CYCLES_RENDER"));

    /* Only allow setting to non-Cycles if Cycles is NOT available.
     * This prevents switching via menu or other means if Cycles is here. */
    if (!has_cycles || is_cycles) {
                                                               ...
                                                               bool *r_free)
{
  ...
  for (type = static_cast<RenderEngineType *>(R_engines.first); type; type = type->next) {
    if (type->idname &&
        (BLI_strcaseeq(type->idname, "CYCLES") || BLI_strcaseeq(type->idname, "CYCLES_RENDER")))
    {
      has_cycles = true;
      break;
    }
  ...
  for (type = static_cast<RenderEngineType *>(R_engines.first); type; type = type->next, a++) {
    ...
    /* If Cycles is available, hide all other engines */
    if (has_cycles && !is_cycles) {
      continue;
    }
...
static int rna_RenderSettings_engine_get(PointerRNA *ptr)
{
  ...
  /* Check if current engine is valid and allowed */
  for (type = static_cast<RenderEngineType *>(R_engines.first); type; type = type->next, a++) {
    bool is_cycles = type->idname && (BLI_strcaseeq(type->idname, "CYCLES") ||
                                      BLI_strcaseeq(type->idname, "CYCLES_RENDER"));

    if (STREQ(type->idname, rd->engine)) {
      /* If found but hidden (e.g. EEVEE when Cycles is present), treat as invalid */
      if (has_cycles && !is_cycles) {
        break;
      }
      return a;
  ...
  /* Fallback: Engine not found or prohibited. Find Cycles or first allowed. */
  a = 0;
  for (type = static_cast<RenderEngineType *>(R_engines.first); type; type = type->next, a++) {
    bool is_cycles = type->idname && (BLI_strcaseeq(type->idname, "CYCLES") ||
                                      BLI_strcaseeq(type->idname, "CYCLES_RENDER"));
...
static void rna_FFmpegSettings_codec_update(Main * /*bmain*/, Scene * /*scene*/, PointerRNA *ptr)
{
  ...
  if (!MOV_codec_supports_crf(codec_data->codec_id_get())) {
    /* Constant Rate Factor (CRF) setting is only available for some codecs. Change encoder
     * quality mode to CBR for others. */
